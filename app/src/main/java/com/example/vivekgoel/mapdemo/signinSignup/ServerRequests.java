...
public class ServerRequests {
    ...
    public static final String SERVER_ADDRESS ="http://smartparksj.com/";
...
//            dataToSend.add(new BasicNameValuePair("checkbox", user.checkbox + ""));
    ...
    public class FetchUserDataAsyncTask extends AsyncTask<Void,Void,User> {
        ...
        protected User doInBackground(Void... params) {           //send request to the server the user which has username n pwd as follows
            ...
            dataToSend1.add(new BasicNameValuePair("email", user.email));
...
//                    int checkbox = jObject.getInt("checkbox");
                    returnedUser = new User(firstName, lastName,user.email, user.password, phnum);
//                    returnedUser = new User(firstName, lastName,user.email, user.password, phnum,checkbox);//we already know the username n password so we reuse them
